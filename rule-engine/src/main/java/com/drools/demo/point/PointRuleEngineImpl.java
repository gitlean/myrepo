...
public class PointRuleEngineImpl implements PointRuleEngine {
	...
	public void initEngine() {
		...
		try {
			// Drools中可以使用PackageBuilder类来编译drl文件 (规则脚本)
	...
	public void executeRuleEngine(final PointDomain pointDomain) {
		...
		/**
		 * 规则有了，交互的对象也有了，我们需要实现一个workingMemory来装载这些对象进行运算。 在Drools5
		 * 当中提供了两个对象与规则引擎进行交互：StatefulKnowledgeSession和StatelessKnowledgeSession。
		 * 本例中使用了StatefulKnowledgeSession进行交互。
		 */
	...
	private List<String> getTestDrlFile() {
		...
		drlFilePath.add("D:/projects/git/myrepo/rule-engine/src/main/java/com/drools/demo/point/addpoint.drl");
		drlFilePath.add("D:/projects/git/myrepo/rule-engine/src/main/java/com/drools/demo/point/subpoint.drl");
		return drlFilePath;
