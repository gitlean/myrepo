...
<body>
	...
	<p>-----------------------------------------------------------------------</p>
	<div id="mainMap2" style="width: 1000px; height: 800px"></div>
...
<script type="text/javascript">
	...
	$.get('map/jiangsu.json', function(mapJson) { //请求 "map/shandong.json"  返回数据 mapJson

		echarts.registerMap('jiangsu', mapJson); //"shandong"是标记作用还是必须是山东?  只是一个标记

		var chart = echarts.init(document.getElementById('mainMap2'));//在id为mainMap的dom元素中显示地图
		...
		option = {
			...
			series : [ {
				type : 'map',
				map : 'jiangsu',//要和echarts.registerMap（）中第一个参数一致
				label : {
					normal : {
						show : true
					},
